...
bool VulkanTextureNode::buildTexture(const QSize &size) {
...
void VulkanTextureNode::freeTexture() {
    if (m_texture) {
...
bool VulkanTextureNode::createRenderPass() {
...
bool VulkanTextureNode::initialize() {
    ...
    m_physDev = *static_cast<VkPhysicalDevice *>(rif->getResource(m_window,
...
void VulkanTextureNode::sync() {
    ...
    m_t = float(static_cast<VulkanTextureItem *>(m_item)->t());
}

void VulkanTextureNode::render() {
    if (!m_initialized)
        return;
...
void VulkanTextureNode::prepareShader(Stage stage) {
    QString filename;
    if (stage == VertexStage) {
        filename = QLatin1String(":/scenegraph/vulkantexture/squircle.vert.spv");
    } else {
        Q_ASSERT(stage == FragmentStage);
        filename = QLatin1String(":/scenegraph/vulkantexture/squircle.frag.spv");
    }
    QFile f(filename);
